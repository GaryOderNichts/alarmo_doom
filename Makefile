...
BUILD		:=	build
SOURCES		:=	source source/chocdoom source/libalarmo
INCLUDES	:=	source miniz
DATA		:=	data
...
CFLAGS	:=	$(MACHDEP) $(INCLUDE) -O3 -DMINIZ_NO_STDIO -DMINIZ_NO_TIME
...
$(TOPDIR)/a.bin: $(OUTPUT).bin
	...
	$(SILENTCMD)python3 $(TOPDIR)/ciphtool.py encrypt $< $@
...
#-------------------------------------------------------------------------------
# This will compress the wad and create a header file for it
#-------------------------------------------------------------------------------
%_wad.h :	%.wad
#-------------------------------------------------------------------------------
	@echo $(notdir $<)
	@zlib-flate -compress < $< > $(notdir $<)
	@xxd -i $(notdir $<) > $@
	@echo -n "#define doom1_wad_unc_len " >> $@
	@stat -L -c %s $< >> $@
